...
def task_check_survey():
    ...
    with open(config['config'], 'r') as ymlfile:
        cfg = yaml.load(ymlfile, yaml.SafeLoader)
    basepath = os.path.dirname(config['config'])
    file_dep = glob.glob(basepath,os.path.join(cfg['paths']['process'],'*_survey_area.csv'),recursive=True)
    for file in file_dep:
...
def task_concat_check_survey():
    ...
    with open(config['config'], 'r') as ymlfile:
        cfg = yaml.load(ymlfile, yaml.SafeLoader)
    basepath = os.path.dirname(config['config'])
    file_dep = glob.glob(basepath,os.path.join(cfg['paths']['process'],'*_survey_area_summary.csv'),recursive=True)
