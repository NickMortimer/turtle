...
#                             ]
#                         }
#                         save_json(new_labelme_json, tile_path.with_suffix(".json"))  # Save JSON for the tile
#         return id
...
#     file_dep = list((config.geturl('output') / 'turtles').glob('**/images/*.JPG'))
#     return {
#         'file_dep' : file_dep,
...
#     dirs = directories = list((config.geturl('output') / 'turtles').glob("**/slices"))
...
#     files = list((config.geturl('output') / 'turtles').rglob("annotations.csv"))
